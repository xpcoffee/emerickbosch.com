...
**TLDR** An "interceptor" is a <Link to="/articles/patterns">pattern</Link> which enables different operations to act in sequence on the same object.
...
we may want to perform authentication, <Link to="/articles/load-shedding-throttling">request-throttling</Link> and authorization in that order for all requests.
