...
import SequenceDiagram from "./images/strategy-sequence.svg"

**TLDR** A "strategy" is a <Link to="/articles/patterns">pattern</Link> which provides a way to define and exchange multiple implementations for processes.
...
During runtime, based on application context, we can then determine which implementation to use e.g. using a <Link to="/articles/patterns">factory</Link> and we set the appropriate implementation for the application to use.
