...
class BoardGame {
    ...
    private getInitialBoard({numberOfPlayers}: {numberOfPlayers: number}): Board {
        switch(numberOfPlayers) {
            ...
            default:
                throw new Error(`Cannot create a new board for ${numberOfPlayers} players`);
        }
...
```typescript
function getLoggingInterceptor({devMode, remoteLogging}: AppConfig): Interceptor {
    if(devMode) {
