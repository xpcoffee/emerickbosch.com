...
function toNote(noteString: string): Note {
    ...
    const boundedInterval = (modifiedInterval > 0 ? modifiedInterval : modifiedInterval + 12) % 12
...
function getModalScale(scaleDegree: number, key: Note): string[] {
    try {
        ...
        for (let i = 0; i < scaleBase.length - 1; i++) {
            ...
            currentModifier = currentModifier + intervalsForward - note.intervalFromPreviousNote
...
function getChordNames(roots: string[], thirds: string[], fifths: string[]) {
    ...
    for (let i = 0; i < roots.length; i++) {
        ...
        const fifthInterval = fifth.interval - third.interval + (fifth.interval > third.interval ? 0 : 12)
        ...
        if (thirdIntervalName === "iii" && fifthIntervalName === "III") {
            ...
            chordNames.push("??")
            console.log(`Unkown triad 3rd: ${thirdIntervalName}, 5th: ${fifthIntervalName}`)
        }
    }
    return chordNames;
}
...
const ModalScale = () => {
    ...
    const chordNames = getChordNames(modalScale, modalScaleThird, modalScaleFifth)
